      ...
      <Container className="JourneyView">
        <div className="border">
          {!journey ? (<Row>
            ...
            <Col xs="12" md="3">
              <JournalPagesList journey={journey} activePage={ Util.getNextPathElement(this.props.location, this.props.match) } />
            </Col>
            <Col xs="12" md="9">
              <Switch>
                <Route path={`${this.props.match.url}/overview`} render={({ match, location }) => <JournalOverviewPage journey={journey} match={match} location={location} />} />
                <Route path={`${this.props.match.url}/:day`} render={({ match, location }) => {
                  const page = journey.findPage(match.params.day);

                  return (
                    <JournalPage journey={journey} page={page} match={match} location={location} />
                  );
                }} />
                <Redirect path={this.props.match.url} to={`${this.props.match.url}/overview`} />
              </Switch>
            </Col>
          </Row>)}
        </div>
